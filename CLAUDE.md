...
- Updates: GitHub Releases (`build.publish` in package.json). Windows and Developer ID macOS builds use electron-updater. Ad-hoc macOS builds use a verified ZIP and same-user staged app replacement when writable, with manual DMG fallback otherwise. Check → download with progress → install on restart, all from Settings and MCP `app_updates`. Downloads never start on their own. The app version is baked into both main and renderer via `__APP_VERSION__` because `app.getVersion()` returns Electron's version in an unpackaged build
...
### Large-history startup reliability
History stores request only their own session. The shared preload/main transport reads and writes bounded chunks instead of copying the entire history catalog into every startup request. Writes use a temporary file and atomic rename, so interrupted transfers preserve the previous history. UI and MCP still read the same live stores; embedded reference snapshots and legacy histories remain intact. `node scripts/test-large-history-app.mjs` checks startup with a synthetic 260 MiB gallery, exact large-history save/read, and live UI/MCP visibility without paid requests.


### Reference memory and renderer recovery
Gallery metadata retains reference paths instead of inline image bytes. Startup migrates gallery and generation-option reference snapshots (including video start frames) to content-addressed internal files without re-encoding, preserving labels/order and a byte-exact pre-migration history backup. The shared generation hooks retain new references once before submission and hydrate only active references for providers. Gallery saves coalesce overlapping requests; legacy dimension inspection decodes one original at a time. UI and MCP use the same paths, reuse and metadata. A renderer-process crash records only reason/time/exit code and offers a native reload dialog; it never replays paid requests. In-flight provider jobs are not automatically restored by reloading. `node scripts/test-reference-history-app.mjs` verifies a synthetic 260 MiB migration, exact reference bytes, repeated mocked generation/persistence/reuse and the recovery notice.
