...
Place references **inside the relevant prompt sentence**, just like the app's mention chips: `Use the person from [@Timo], wearing the outfit from [Image 1], in the room from [Image 2].` `collections` returns each collection's exact `promptReference`; also pass its ID in `collectionIds` to attach its images. Individual entries of `references` are `[Image 1]`, `[Image 2]`, `[Image 3]`, etc., in array order, whether imported from files/URLs or generated earlier. Collections and automatic editing references do not shift that numbering. Attaching an ID alone does not insert an inline mention; writing a mention alone does not attach media. `get_draft` returns the actual editor references and their exact markers. `preview_generation.referenceMentions` shows each attachment's marker and whether the prompt contains it.

Video generation currently uses a single start image. Describe its subjects and motion directly in the prompt, for example `The person in the start image waves`. To use multiple visual references for a video, first compose an image with the inline references above, then animate that image. End-frame and video/audio-reference inputs are currently unavailable in both the UI and MCP.
