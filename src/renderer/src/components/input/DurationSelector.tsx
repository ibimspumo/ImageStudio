    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <ComposerPopover className="bg-surface-3 border border-border-base rounded-xl shadow-sm p-2 animate-scale-in min-w-[180px]">
            <div className="px-2 pb-1.5">
              <span className="text-[12px] font-medium text-text-muted">Dauer</span>
            </div>

            {useSlider ? (
              <div className="px-2 py-2 flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-text-muted">{min}s</span>
                  <span className="text-[13px] font-semibold text-text-primary tabular-nums">{value}s</span>
                  <span className="text-[12px] text-text-muted">{max}s</span>
                </div>
                <input
                  type="range"
                  min={min}
