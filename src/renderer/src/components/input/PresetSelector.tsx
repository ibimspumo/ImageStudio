  ...
  return (
    <div className="relative shrink-0">
      <button
        ...
        className={cn(
          'no-drag flex items-center gap-1.5 h-8 px-2.5 rounded-lg border text-[12px] font-medium transition-all',
          activePreset
      ...
      >
        ...
        <span className="max-w-[80px] truncate">{activePreset ? activePreset.name : 'Stilvorlage'}</span>
      ...
      {open && (
        <>
          ...
          <ComposerPopover className="w-[240px] bg-surface-3 border border-border-base rounded-xl shadow-sm p-1.5 animate-scale-in max-h-[320px] overflow-y-auto">
            {/* None option */}
            <button
              onClick={() => { setActivePreset(null); setOpen(false) }}
              className={cn(
                'w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-[12px] text-left transition-colors',
                !activePresetId
            ...
            >
              ...
              <span className="flex-1">Keine Stilvorlage</span>
            ...
            {presets.map((preset) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-[12px]">{preset.name}</div>
                  <div className="text-[12px] text-text-muted truncate">{preset.suffix.slice(0, 50)}...</div>
                </div>
            ...
            {onManageClick && (
              <>
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-[12px] text-text-muted hover:text-text-secondary hover:bg-surface-4 transition-colors"
                >
