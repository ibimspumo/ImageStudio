...
import { Image, Film, Youtube, Hexagon, Grid2X2, Layers, SlidersHorizontal, Folder, Activity, Settings, Plus, X } from 'lucide-react'
...
export const SECTION_LABELS: Record<StudioSection, string> = { image:'Bild', video:'Video', thumbnail:'Thumbnail', logo:'Logo', library:'Mediathek', references:'Referenzen', styles:'Stile & Vorlagen', projects:'Projekte', activity:'Aktivität', settings:'Einstellungen' }
...
export function StudioSidebar({ section, onNavigate, onSelectWorkspace, onSelectProject, open, onClose }: Props) {
  ...
  return <>
    ...
    <aside className={`studio-sidebar ${open?'is-open':''}`} aria-label="Studio-Navigation">
      ...
      <nav className="studio-sidebar-nav" aria-label="Hauptnavigation"><p className="studio-nav-label">Erstellen</p>{nav('image',Image)}{nav('thumbnail',Youtube)}{nav('video',Film)}{nav('logo',Hexagon)}<div className="h-5"/>{nav('library',Grid2X2)}{nav('references',Layers)}{nav('styles',SlidersHorizontal)}<div className="h-5"/><div className="flex items-center studio-nav-label"><span>Projekte</span><button className="ml-auto p-1 rounded hover:bg-surface-4" title="Projekte verwalten" aria-label="Projekte verwalten" onClick={()=>onNavigate('projects')}><Plus className="w-4 h-4"/></button></div>{entries.map(p=><button key={p.id} className="studio-nav-item" title={`Videoprojekt: ${p.title}`} onClick={()=>{onSelectProject(p.id);onClose()}} onDragOver={e=>{if(e.dataTransfer.types.includes('application/x-imagestudio'))e.preventDefault()}} onDrop={e=>drop(e,p.id,'project')}><span className="w-2 h-2 rounded-sm shrink-0" style={{background:p.color}}/><span className="truncate flex-1">{p.title}</span>{section==='thumbnail'&&activeProject===p.id&&<span className="w-1.5 h-1.5 bg-accent-main rounded-full"/>}</button>)}{workspaces.slice(-3).map(w=><button key={w.id} className="studio-nav-item" title={`Arbeitsordner: ${w.name}`} onClick={()=>{onSelectWorkspace(w.id);onClose()}} onDragOver={e=>{if(e.dataTransfer.types.includes('application/x-imagestudio'))e.preventDefault()}} onDrop={e=>drop(e,w.id,'workspace')}><Folder className="w-4 h-4 shrink-0"/><span className="truncate flex-1">{w.name}</span>{activeWorkspace===w.id&&section!=='thumbnail'&&<span className="w-1.5 h-1.5 bg-accent-main rounded-full"/>}</button>)}{nav('projects',Folder)}</nav>
