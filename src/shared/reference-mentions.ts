...
export const REFERENCE_PROMPT_GUIDANCE = {
  ...
  sources: 'The same inline image references apply to imported local images, images downloaded from URLs, previous generation results, and additional images in editing chats or inpainting. Attach each image through the tool input as well as mentioning it. Assign each reference a clear role; mention it again when needed.',
  chat: 'In editing chats, refer to the automatically attached latest result as the previous image (current version). Additional references start at [Image 1]; the automatic image does not shift their numbering.',
